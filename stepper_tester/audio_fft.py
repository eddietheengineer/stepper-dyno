...
def processfile(wav_dir, file_name):
	samp_rate = 44100 # 44.1kHz sampling rate
	chunk = 4096
	...
	annot = ax1.annotate('Freq: %2.1f'%(f_vec[max_loc]),xy=(f_vec[max_loc],fft_data[max_loc]),\
						xycoords='data',xytext=(0,30),textcoords='offset points',\
						arrowprops=dict(arrowstyle="->"),ha='center',va='bottom')
	export_dir = wav_dir + 'FFT/'
	
	if not os.path.exists(export_dir):
		os.makedirs(export_dir)
	export_file_name = file_name[0:-4] + '.png'
	export_file = export_dir + export_file_name
	plt.savefig(export_file,dpi=300,facecolor='#FCFCFC')
	#plt.show()
	plt.clf()
	plt.close()